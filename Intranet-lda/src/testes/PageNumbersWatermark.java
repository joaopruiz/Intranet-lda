...
public class PageNumbersWatermark extends PdfPageEventHelper {
    ...
    public static void main(String args[]) {
        try {
            ...
            PdfWriter writer = PdfWriter.getInstance(doc, new FileOutputStream("src/testes/pageNumbersWatermark.pdf"));
    ...
    public void onOpenDocument(PdfWriter writer, Document document) {
        try {
            ...
            headerImage = Image.getInstance("src/testes/logo.gif");
            table = new PdfPTable(2);
            Phrase p = new Phrase();
            Chunk ck = new Chunk("INTRANET DPF\n", new Font(Font.TIMES_ROMAN, 16, Font.BOLDITALIC, Color.blue));
            p.add(ck);
    ...
    public void onStartPage(PdfWriter writer, Document document) {
        if (writer.getPageNumber() < 3) {
            ...
            cb.beginText();
            cb.setFontAndSize(helv, 48);
            cb.showTextAligned(Element.ALIGN_CENTER, "Minha marca d'água " + writer.getPageNumber(), document.getPageSize().width() / 2, document.getPageSize().height() / 2, 45);
            cb.endText();
            cb.restoreState();
        }
    }
